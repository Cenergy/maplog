...
class PlotService extends EventTarget {
  ...
  startGetNavigationData(selector = 'body') {
    this._waypoints = [];
    this._routePoints = [];
    ...
    this._mapSdk.map.on('click', (e) => {
      const selectValue = document.querySelector(`${selector}`);
  ...
  _finishGetWaypoint() {
    ...
    setTimeout(() => {
      this.dispatchEvent(
        new RouteFinishEvent(EVENT_ROUTE_GETWAYPOINTS_FINISHED, this._routePoints),
      );
    }, 1000);
  }
}
