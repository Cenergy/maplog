 ...
 * const option = {
  ...
  * const result = {distance: 11137.4,
  ...
  async getNavigationData(option) {
    ...
    if (!this._cacheRoutes.get(coordinates.toString())) {
      console.log("rd: NavigationService -> getNavigationData -> first request!", coordinates)
      const navitgationData = await this._getNavigationDatas(...coordinates);
      this._cacheRoutes.set(coordinates.toString(), navitgationData);
    }

    return this._cacheRoutes.get(coordinates.toString());
  }
