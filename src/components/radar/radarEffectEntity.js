...
import layerZIndexHelper, { LOWER_FLOOR } from '../layerZIndexHelper';
...
export default class RadarEffectEntity {
    ...
    addToMap(map) {
        ...
        const zIndexLayerID = layerZIndexHelper.getLayerZIndex(LOWER_FLOOR);
        map.addLayer(
            {
                id: this._id,
                type: 'raster',
    ...
    static _getScanRadius(spreadRank) {
        if ((typeof spreadRank !== 'number' && spreadRank < 0) || spreadRank > 11) return 1;
        return spreadRank * 0.1;
    }
}
