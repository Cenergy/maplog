...
class ShadeWapper {
  ...
  remove() {
    ...
    tbService.tb.remove(this._shadeTemplate);
    if (this.drawable.title !== '' && this.drawable.type === 'sphere') {
      this.popup.remove();
    }
    this._map.repaint = true;
  ...
  _drawBox() {
    ...
    group.add(shadeObj);
    this._shadeTemplate = tbService.tb.Object3D({ obj: group }).setCoords(this.cache.coordinate);
    tbService.tb.add(this._shadeTemplate);
  ...
  _drawSphere() {
    ...
    this._shadeTemplate = tbService.tb.Object3D({ obj: shadeObj }).setCoords(this.cache.coordinate);
