...
export default class PointLayerHandler extends LayerHandlerBase {
  ...
  _construtInteration() {
    ...
    this._mapSdk.map.on('click', POINT_NAME_POINT_DATASOURCE, (e) => {
      ...
      el.innerHTML = jsonToHtml(properties);
      new mapboxgl.Popup({ offset: 36, anchor: 'bottom', className: 'pointLayer' })
        .setLngLat(coordinates)
        .setDOMContent(el)
        .addTo(this._mapSdk.map);
    });
  ...
  async _loadLayerIcons() {
    ...
    this._pointLayers.forEach((layer) => {
      const imagePath = `${dataService.baseUrl}images/layerIcons/${layer.typeId}.png`;
      promises.push(
        new Promise((res, rej) => {
