...
function getArrowHeadPoints(points, tailLeft, tailRight) {
    ...
    const headEndPnt = plotUtil.getThirdPoint(points[points.length - 2], headPnt, 0, headHeight, true);
    const neckEndPnt = plotUtil.getThirdPoint(points[points.length - 2], headPnt, 0, neckHeight, true);
...
function getArrowBodyPoints(points, neckLeft, neckRight, factor) {
    const allLen = plotUtil.wholeDistance(points);
    const len = plotUtil.getBaseLength(points);
    const tailWidth = len * factor;
...
function generateTailedAttackArrow(option) {
    ...
    swallowTailPnt = plotUtil.getThirdPoint(bonePnts[1], bonePnts[0], 0, len, true);
    ...
    const pnts = [leftPnts.concat(headPnts, rightPnts.reverse(), [swallowTailPnt, leftPnts[0]])];
    return pnts[0];
}

export default { generateTailedAttackArrow };
